...
archivo = "RSAM_crossval_strata_RF_simula"   ## name of the files to save results
```
...
train1 <- train1 %>% select(-c(rep,ID_fact,STATUS_fact))
train2 <- train2 %>% select(-c(rep,ID_fact,STATUS_fact))
train3 <- train3 %>% select(-c(rep,ID_fact,STATUS_fact))
